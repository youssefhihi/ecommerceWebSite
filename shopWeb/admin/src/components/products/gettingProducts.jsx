...
const GettingProducts = ({ searchQuery }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`${BASE_URL}/api/products?page=${page}&limit=6&search=${search}`);
        ...
        <>
            ...
            {loading && isGridView2 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-xs md:max-w-4xl mx-auto">
                    {[...Array(3)].map((_, index) => (
                        <div key={index} className="animate-pulse relative flex flex-col rounded-xl p-3 md:w-full mx-auto  ">
            ...
            {isGridView && (
                <div className='flex flex-col gap-5 max-w-xs md:max-w-4xl mx-auto'>
                    {products.map((product, index) => (
                        <div key={index} className=" relative flex flex-col md:flex-row md:space-x-5 space-y-3 md:space-y-0 rounded-xl shadow-lg p-3 w-full mx-auto border border-white bg-white">
            ...
            {isGridView2 && (
                <div className='grid grid-cols-1 md:grid-cols-3 gap-5 max-w-xs md:max-w-4xl mx-auto'>
                    {products.map((product, index) => (
                        <div key={index} className="relative flex flex-col rounded-xl shadow-lg p-3 md:w-full mx-auto border border-white bg-white">
                        ...
                        {Array.from({ length: totalPages }, (_, i) => (
                            <li key={i}>
                                <button
                                    onClick={() => setPage(i + 1)}
                                    disabled={i + 1 === page}
                                    className={`h-10 px-5 text-white transition-colors duration-150 ${i + 1 === page ? 'bg-[#3cf23c]' : 'bg-gray-200'} focus:shadow-outline`}
                                >
                                    {i + 1}
                                </button>
                            </li>
